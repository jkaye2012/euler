...
struct Problem11Traversal<'a> {
    grid: &'a StaticGrid<u64, 400, 20>,
    count: usize,
}

impl<'a> Problem11Traversal<'a> {
    pub fn traverse(&self, col: usize, row: usize) -> u64 {
        let mut result = 0u64;
        if col + self.count < self.grid.width() {
            let mut calc = 1u64;
            for o in 0..self.count {
                calc *= self.grid.at(col + o, row);
            }
            result = result.max(calc);
        }
        if row + self.count < self.grid.height() {
            let mut calc = 1u64;
            for o in 0..self.count {
                calc *= self.grid.at(col, row + o);
            }
            result = result.max(calc);
        }
        if col + self.count < self.grid.width() && row + self.count < self.grid.height() {
            let mut calc = 1u64;
            for o in 0..self.count {
                calc *= self.grid.at(col + o, row + o);
            }
            result = result.max(calc);
        }
        if col >= self.count && row + self.count < self.grid.height() {
            let mut calc = 1u64;
            for o in 0..self.count {
                calc *= self.grid.at(col - o, row + o);
            }
...
fn main() {
    let grid = StaticGrid::<u64, 400, 20>::new(GRID);
    let traversal = Problem11Traversal {
        grid: &grid,
        count: 4,
    };
    let result = grid.traverse(
        |col, row| traversal.traverse(col, row),
        |acc, val| acc.max(val),
    );
    println!("{result}");
}
